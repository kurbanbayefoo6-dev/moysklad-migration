...
import { validateInventoryPayload } from '../validators/inventoryValidator.js'
...
function buildInventoryPositionIdentity(position) {
	return [
...
function buildInventoryBusinessIdentity(inventory) {
	return [
		normalizeString(inventory?.moment),
		String(Boolean(inventory?.applicable)),
		getReferenceKey(inventory?.store),
		getRows(inventory?.positions)
			.map(buildInventoryPositionIdentity)
			.sort()
			.join('||'),
	].join('|')
}
...
async function loadOldInventories() {
...
async function dryRunInventoryMigration() {
	const inventories = await loadOldInventories()
	const stats = createDryRunStats()
	stats.loaded = inventories.length

	for (const inventory of inventories) {
		let payload
		try {
			payload = await withApiRetries(
				() => inventoryMapper.map(inventory),
				`dry-run map Inventory ${getDocumentNumber(inventory)}`,
			)
			validateInventoryPayload(payload)
			stats.built += 1
		} catch (error) {
			addPreflightError(stats.validationErrors, inventory, error)
			continue
		}

		try {
			validateNoOldReferences(payload, inventory)
		} catch (error) {
			addPreflightError(stats.oldReferenceErrors, inventory, error)
		}
	}
	...
	console.log(`Inventory loaded: ${stats.loaded}`)
	console.log(`Inventory payloads built: ${stats.built}`)
	console.log('')
...
async function migrateInventories() {
	const oldInventories = await loadOldInventories()
	const newInventories = await withApiRetries(
		() =>
	...
	stats.total = oldInventories.length

	for (const inventory of oldInventories) {
		try {
			if (
				inventory.externalCode &&
				newByExternalCode.has(inventory.externalCode)
			) {
				stats.skipped += 1
				continue
			}
			...
			const payload = await withApiRetries(
				() => inventoryMapper.map(inventory),
				`map Inventory ${getDocumentNumber(inventory)}`,
			)
			validateInventoryPayload(payload)
			const existingByBusinessIdentity = newInventories.find(
				newInventory =>
					buildInventoryBusinessIdentity(newInventory) ===
					buildInventoryBusinessIdentity(payload),
			)
			...
			const created = await withApiRetries(
				() => inventoryRepository.create(payload, { client: 'new' }),
				`CREATE Inventory ${getDocumentNumber(inventory)}`,
			)
			...
			addFailure(stats, inventory, error)
		}
	}
...
export async function migrateInventory({ execute = false } = {}) {
	if (!execute) {
		return dryRunInventoryMigration()
	}

	return migrateInventories()
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
	migrateInventory({ execute: process.argv.includes('--execute') }).catch(error => {
		console.log('Inventory migration failed')
